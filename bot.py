...
async def run_bot(token: str):
    ...
    # -------- HELP --------
        elif text == "အကူညီ" and is_admin_or_owner(user_id, data["BOT_ADMINS"]):
            await message.delete()
            await message.reply_text("""<pre>
📌 CALL COMMANDS(gp)
All - Call all members
Callone - Call all admins
Call - Call all active members
ရပ် - Stop call
...
အရှိန် - Set spam speed (0.1 - 10 sec)

📌 SPAM TEXT MANAGER (owner)
စာထည့် text - Add spam text
စာဖြတ် text - Remove spam text
စာlist - Show spam text list
...
📌 BROADCAST
file - Broadcast reply message

📌 OTHER
ဘာသာပြန် - Translate reply message
