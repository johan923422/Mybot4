...
async def call_members_online(client, message: Message, mode: str, optional_text: str = ""):
    ...
    try:
        ...
        for i in range(0, len(mention_list), batch_size):
            ...
            await client.send_message(chat_id, msg_text, disable_web_page_preview=True, parse_mode=ParseMode.HTML)
            await asyncio.sleep(1)

    except Exception as e:
        await client.send_message(chat_id, f"Call error: {e}", parse_mode=ParseMode.HTML)
...
async def run_bot(token: str):
    ...
    async def handler(client, message: Message):
        ...
        if text == "ရပ်":
            await message.delete()
            call_running[chat_id] = False
            await message.reply_text("Call ရပ်ပြီးပါပြီသခင်", parse_mode=ParseMode.HTML)
            return
        ...
        if text in ["mtရိုက်","ရိုက်သတ်"] and message.reply_to_message and is_admin_or_owner(user_id):
            ...
            async def spam_worker(tag_user: bool):
                while spam_running.get(chat_id):
                    if tag_user:
                        await app.send_message(chat_id, f"<a href='tg://user?id={user.id}'>{user.first_name}</a>\n{SPAM_TEXT}", disable_web_page_preview=True, parse_mode=ParseMode.HTML)
                    else:
                        await app.send_message(chat_id, SPAM_TEXT, disable_web_page_preview=True, parse_mode=ParseMode.HTML)
        ...
        if text == "ခွင့်လွှတ်လိုက်" and is_admin_or_owner(user_id):
            ...
            await message.reply_text("သခင့်အမိန့်အရဖာသည်မသားကိုလွတ်မြောက်ခွင့်ပေးလိုက်ပါပြီ", parse_mode=ParseMode.HTML)
            return
        ...
        if text.startswith("အရှိန်") and is_admin_or_owner(user_id):
            ...
            try:
                ...
                if 1 <= speed <= 5:
                    ...
                    await message.reply_text(f"အမြန်နှုန်း {speed} သတ်မှတ်ပြီးပါပြီ", parse_mode=ParseMode.HTML)
            except:
                await message.reply_text("အမှားရှိပါတယ်၊ 1-5 အတွင်းနံပါတ်သာအသုံးပြုပါ။", parse_mode=ParseMode.HTML)
            return
        ...
        for word in FILTER_WORDS:
            if word in text.lower() and not is_admin_or_owner(user_id):
                await message.delete()
                await message.reply_text(f"{message.from_user.first_name} ဖာသည်မသား", parse_mode=ParseMode.HTML)
                return
        ...
        if message.entities:
            for ent in message.entities:
                if ent.type in ['url','text_link'] and not is_admin_or_owner(user_id):
                    await message.delete()
                    await message.reply_text(f"{message.from_user.first_name} မင်းအမေပိုင်တဲ့ဖာရုံမဟုတ်ဘူး", parse_mode=ParseMode.HTML)
                    return
...
ဖြုတ်
        ...
        if text == "list" and is_admin_or_owner(user_id):
            ...
            await message.reply_text(f"📜 Bot Admins:\nOwner: {OWNER_ID}\nAdmins: {', '.join(names)}", parse_mode=ParseMode.HTML)
            return

        # --- File broadcast ---
        if message.document:
            last_doc[chat_id] = message
            await message.reply_text("Reply 'file' to broadcast.", parse_mode=ParseMode.HTML)
            return
        if text.lower() == "file" and is_admin_or_owner(user_id):
            await message.delete()
            if chat_id not in last_doc:
                await message.reply_text("No file found.", parse_mode=ParseMode.HTML)
                return
            ...
            try:
                ...
                await message.reply_text("Not a text file.", parse_mode=ParseMode.HTML)
                return
            for gp_chat in GP_CHAT_IDS:
                await app.send_message(gp_chat, f"📦 <b>File Broadcast</b>\n👤 From: {doc_msg.from_user.first_name}\n📝 Content:\n<pre>{text_content}</pre>", parse_mode=ParseMode.HTML)
        ...
        if text == "Info" and message.reply_to_message and is_admin_or_owner(user_id):
            await message.delete()
            await message.reply_text(f"👤 {message.reply_to_message.from_user.first_name}\n🆔 {message.reply_to_message.from_user.id}", parse_mode=ParseMode.HTML)
            return

    await app.start()
    print(f"Bot started: {token}")
    await asyncio.Event().wait()  # replaced app.idle()
