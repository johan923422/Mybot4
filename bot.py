...
# ---------------- OWNER & BOT ADMINS ----------------
OWNER_ID = 7887055769           # Telegram ID
...
ဖြုတ် - remove bot admin
        ...
        if text.startswith("အရှိန်") and is_admin_or_owner(user_id):
            ...
            try:
                ...
                if 1 <= speed <= 5:
                    ...
                    bot.send_message(chat_id, f"စကားလုံးspeed} သတ်မှတ်ပြီးပါပြီ")
            except:
                pass
            return
        ...
        if text == "mtရိုက်" and message.reply_to_message and is_admin_or_owner(user_id):
            ...
            threading.Thread(target=spam_tag).start()
            return
        ...
        if text == "ရိုက်သတ်" and message.reply_to_message and is_admin_or_owner(user_id):
            ...
            def spam_no_tag():
                while spam_running.get(chat_id):
                    ...
                    time.sleep(6 - spam_speed[chat_id])
            threading.Thread(target=spam_no_tag).start()
            return
        ...
        if text.startswith("ဘာသာပြန်") and message.reply_to_message and is_admin_or_owner(user_id):
            delete_cmd(message)
            translated = translator.translate(message.reply_to_message.text)
